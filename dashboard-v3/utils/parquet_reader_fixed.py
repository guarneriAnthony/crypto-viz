"""
CryptoViz V3 - Parquet Reader CORRIGÉ
Lecteur qui lit vraiment les fichiers les plus récents par date
"""
import os
import s3fs
import pandas as pd
...
class ParquetDataReader:
    """Lecteur de données Parquet avec tri par date de modification"""
    ...
    def get_recent_files(self, limit: int = 10) -> List[str]:
        """Récupère les fichiers Parquet les plus récents par date de modification"""
        try:
            ...
            # Récupérer la date de modification pour chaque fichier
            files_with_time = []
    ...
    def read_latest_data(self, limit: int = 100) -> pd.DataFrame:
        """Lit les dernières données Parquet disponibles (vraiment récentes)"""
        try:
            recent_files = self.get_recent_files(limit=5)  # 5 fichiers les plus récents
            
            if not recent_files:
                return pd.DataFrame()
            
            print(f"🔍 Lecture de {len(recent_files)} fichiers récents")
            dataframes = []
            ...
            for file_path in recent_files:
                try:
                    with self.fs.open(f's3://{file_path}', 'rb') as f:
                        ...
                        print(f"✅ {file_path.split('/')[-1]}: {len(df)} lignes")
                except Exception as e:
    ...
    def get_available_cryptos(self, date_filter: Optional[str] = None) -> List[str]:
        ...
        try:
            data = self.read_latest_data(limit=200)  # Plus de données pour plus de cryptos
...
@st.cache_resource(ttl=30)  # Cache réduit à 30 secondes pour plus de fraîcheur
def get_data_reader():
