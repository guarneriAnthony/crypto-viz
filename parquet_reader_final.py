"""
CryptoViz V3 - Parquet Reader DÉFINITIF avec TRI PAR DATE
"""
import os
import s3fs
import pandas as pd
...
class ParquetDataReader:
    """Lecteur de données Parquet avec tri par date de modification (pas alphabétique)"""
    ...
    def get_recent_files(self, limit: int = 15) -> List[str]:
    ...
    def read_latest_data(self, limit: int = 500) -> pd.DataFrame:
        ...
        try:
            recent_files = self.get_recent_files(limit=15)  # 15 plus récents par DATE
...
@st.cache_resource(ttl=30)  # Cache court pour données temps réel
def get_data_reader():
